...
namespace RimWorldAccess_UniversalPatcher
{
    ...
    [HarmonyPatch(typeof(Widgets), "CheckboxLabeled")]
    public static class Widgets_CheckboxLabeled_Patch
    {
        public static bool Prefix(Rect rect, string label, ref bool checkOn)
        {
            if (UniversalAccessState.PendingClickTarget.HasValue)
            {
                var target = UniversalAccessState.PendingClickTarget.Value;
        ...
        public static void Postfix(Rect rect, string label, ref bool checkOn)
        {
            if (Event.current != null && Event.current.type == EventType.Repaint)
            {
                UniversalAccessState.ElementsCurrentFrame.Add(new UIElement
                {
                    Text = string.IsNullOrEmpty(label) ? "Checkbox" : TranslationEngine.Translate(label),
                    Rect = rect,
                    Type = "Checkbox",
                    IsChecked = checkOn
                });
    ...
    [HarmonyPatch(typeof(Widgets), "TextField", new Type[] { typeof(Rect), typeof(string), typeof(int), typeof(System.Text.RegularExpressions.Regex) })]
    public static class Widgets_TextField_Patch
    {
        ...
        public static void Postfix(Rect rect, string text, string __state, ref string __result)
        {
            if (Event.current != null && Event.current.type == EventType.Repaint)
            {
                UniversalAccessState.ElementsCurrentFrame.Add(new UIElement
                {
                    Text = string.IsNullOrEmpty(__result) ? "Leeres Textfeld" : __result,
                    Rect = rect,
                    Type = "TextField",
                    TextValue = __result
                });
    ...
    public static class Widgets_HorizontalSlider_Patch
    {
        ...
        public static void Postfix(Rect rect, ref float value, string label, ref float __result)
        {
            if (Event.current != null && Event.current.type == EventType.Repaint)
            {
                UniversalAccessState.ElementsCurrentFrame.Add(new UIElement
                {
                    Text = string.IsNullOrEmpty(label) ? "Schieberegler" : TranslationEngine.Translate(label),
                    Rect = rect,
                    Type = "Slider",
                    CurrentValue = __result
                });
            }
        }
    }
    ...
    public static class FloatMenuOption_DoGUI_Patch
    {
        public static bool Prefix(FloatMenuOption __instance, Rect rect, bool colonistOrdering, FloatMenu floatMenu, ref bool __result)
        {
            ...
            {
                var target = UniversalAccessState.PendingClickTarget.Value;
                if (target.Type == "FloatMenuOption" && target.Rect == rect && target.Text == TranslationEngine.Translate(__instance.Label))
                {
                    UniversalAccessState.PendingClickTarget = null;
                    if (!__instance.Disabled)
                    {
                        __instance.action?.Invoke();
    ...
    public class UniversalAccessMenu : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            if (Event.current != null && Event.current.type == EventType.KeyDown)
            {
                ...
                // Pfeil Links / Left Arrow = Slider verringern
                ...
                // Pfeil Rechts / Right Arrow = Slider erhöhen
        ...
        private void EnsureVisible()
        {
            ...
            else if (selectedY + RowHeight > scrollPosition.y + listHeight)
                scrollPosition.y = selectedY + RowHeight - listHeight;
        }
